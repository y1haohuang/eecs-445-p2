...
def evaluate_autoencoder(dataset, get_semantic_label, model, criterion):
    ...
    for c in range(num_classes):
        len_ = 0
        X = dataset[c]
        recon = predictions(model(X))
        truel = np.ones(800)*c
        performance[c] = metrics.accuracy_score(truel, recon)
        overall_sample_num = overall_sample_num + X.shape[0]
        overall_performance = overall_performance + performance[c] * X.shape[0]
    overall_performance = overall_performance / overall_sample_num
    print('Overall performance: {} accuracy'
